...
import { CompetenzaTecnologica } from '../models/resume.model';
...
export class TechnicalSkillsComponent implements OnInit {
  ...
  items: CompetenzaTecnologica[] = [];
  originalItems: CompetenzaTecnologica[] = [];
  editing: boolean[] = [];
  isLoading = true;
  ...
  loadItems(): void {
    this.isLoading = true;
    this.resumeService.getTechnicalSkills(this.cf).subscribe({
      next: (data: any[]) => {
      ...
      error: () => {
        this.isLoading = false;
        this.snackBar.open('Errore recupero competenze tecnologiche', 'OK', { duration: 3000 });
      }
    });
  }
  ...
  addNew(): void {
    const empty: CompetenzaTecnologica = { titolo: '', descrizione: '' };
  ...
  onSave(): void {
    ...
    this.resumeService.updateTechnicalSkills(this.cf, this.items).subscribe({
      ...
      error: () => this.snackBar.open('Errore salvataggio competenze tecnologiche', 'OK', { duration: 3000 })
    });
  }
  ...
  onBack(): void {
    if (this.isDirty()) {
      const ref = this.dialog.open(ConfirmLeaveDialogTech);
...
@Component({
  selector: 'confirm-leave-dialog-tech',
  standalone: true,
...
export class ConfirmLeaveDialogTech {
  private dialogRef = inject(MatDialogRef<ConfirmLeaveDialogTech>);
