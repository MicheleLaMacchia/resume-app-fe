...
import { CompetenzaTrasversale } from '../models/resume.model';
...
export class SoftSkillsComponent implements OnInit {
  ...
  items: CompetenzaTrasversale[] = [];
  originalItems: CompetenzaTrasversale[] = [];
  editing: boolean[] = [];
  isLoading = true;
  ...
  loadItems(): void {
    this.isLoading = true;
    this.resumeService.getSoftSkills(this.cf).subscribe({
      next: (data: any[]) => {
      ...
      error: () => {
        this.isLoading = false;
        this.snackBar.open('Errore recupero competenze trasversali', 'OK', { duration: 3000 });
      }
    });
  }
  ...
  addNew(): void {
    const empty: CompetenzaTrasversale = { titolo: '', descrizione: '' };
  ...
  onSave(): void {
    ...
    this.resumeService.updateSoftSkills(this.cf, this.items).subscribe({
      ...
      error: () => this.snackBar.open('Errore salvataggio competenze', 'OK', { duration: 3000 })
    });
  }
  ...
  onBack(): void {
    if (this.isDirty()) {
      const ref = this.dialog.open(ConfirmLeaveDialogSoft);
...
@Component({
  selector: 'confirm-leave-dialog-soft',
  standalone: true,
...
export class ConfirmLeaveDialogSoft {
  private dialogRef = inject(MatDialogRef<ConfirmLeaveDialogSoft>);
